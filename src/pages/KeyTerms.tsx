    ...
    sections: [
      {
        ...
        terms: [
          ...
          { term: "Shared semantic model", def: "A published semantic model in Power BI Service that multiple reports can connect to, ensuring consistent business logic.", speed: "One dataset → many reports. Promotes consistency. Build permission required to create reports on top.", tip: "Exam may ask when to use a shared model vs. building your own — shared models promote consistency and reduce duplication.", practice: "Use shared models when multiple teams need the same KPIs to avoid metric discrepancies across reports." },
          { term: "Data source credentials", def: "Authentication details (username/password, OAuth, keys) stored securely to access data sources.", speed: "Stored per semantic model/dataflow in Service. Scheduled refresh uses stored credentials. Gateway data sources have centrally stored credentials.", tip: "Credentials are set on the semantic model (or gateway data source) and used for scheduled refresh regardless of the viewer.", practice: "Use a dedicated service account for gateway credentials to avoid refresh failures when staff leave." },
          ...
          { term: "DirectLake", def: "Fabric-only mode that reads Delta Parquet files directly from OneLake — targets near-import performance with near–real-time freshness.", speed: "Fabric only. No data copy. Near-import performance. Near real-time freshness. Falls back to DirectQuery if guardrails are exceeded.", tip: "DirectLake is new in the Jan 2026 syllabus. Know it as Fabric-only, no data copy, falls back to DirectQuery if guardrails exceeded.", practice: "Use DirectLake for large Fabric lakehouses where you need near-real-time data without import refresh overhead." },
      ...
      },
      {
        ...
        emoji: "🔹",
        terms: [
          { term: "Data types", def: "Classification of column values: Whole Number, Decimal, Text, Date, Boolean, etc.", speed: "Set explicitly — always. Auto-detect can miss dates. Set types early (right after source) to preserve folding and correctness.", tip: "Always set data types explicitly — auto-detection can fail, especially with dates in non-US formats.", practice: "Set data types immediately after sourcing to ensure correct transformations and maintain query folding." },
          ...
          { term: "Foreign key", def: "A column in a fact table that references the primary key of a dimension table.", speed: "Many side. Matches dimension PK. Hide after the relationship is built.", tip: "FKs in facts must match the dimension PK data type; mismatches prevent creation or produce no matches.", practice: "Hide all foreign-key columns from report view so users only see meaningful dimension fields." },
          ...
          { term: "Append queries", def: "Stacks two or more queries vertically to combine rows — equivalent to SQL UNION.", speed: "SQL UNION equivalent. Columns align by name; non-matching columns are created and filled with nulls.", tip: "Ensure consistent column names to avoid unexpected nulls; non-matching columns will be created automatically.", practice: "Use append to combine monthly files from a folder — pair with 'From Folder' connector." },
    ...
    sections: [
      {
        ...
        terms: [
          ...
          { term: "Fact tables", def: "Tables containing numeric measures and foreign keys. Represent events or transactions.", speed: "Primarily measures and FKs; keep degenerate identifiers if needed. Typically high row count.", tip: "Avoid descriptive attributes, but keep necessary degenerate identifiers (e.g., Order No.) in facts.", practice: "Remove descriptive columns from fact tables and instead join to dimensions." },
      ...
      },
      {
        ...
        terms: [
          ...
          { term: "DAX query view", def: "Built-in pane in Power BI Desktop for writing and benchmarking DAX queries directly against the model.", speed: "Left rail → DAX query view. Run queries and benchmark execution.", tip: "Use the Run benchmark button to compare two measure versions deterministically.", practice: "Paste a slow visual's DAX from Performance Analyzer and refactor in DAX query view." },
          ...
          { term: "Aggregations", def: "Pre-computed summary tables stored alongside the fact table — Power BI uses them automatically.", speed: "Manage Aggregations in Model view. Works with composite models; Premium not required.", tip: "Best paired with DirectQuery composite models for big data scenarios.", practice: "Add a daily-grain aggregation table over a transaction-grain DirectQuery fact." },
          { term: "Model size reduction", def: "Techniques to shrink model: remove unused columns, use appropriate data types.", speed: "Remove high-cardinality columns (GUIDs, timestamps).", tip: "Removing high-cardinality columns dramatically improves compression and reduces model size.", practice: "Audit with DAX Studio's VertiPaq Analyser." },
    ...
    sections: [
      {
        ...
        terms: [
          ...
          { term: "Format pane", def: "Pane where visual properties (colours, fonts, axes, titles, data labels) are configured.", speed: "Visualizations → Format your visual.", tip: "Use 'Reset to default' to undo a styling experiment in one click.", practice: "Standardise font and title sizes across the report via the Format pane." },
          { term: "Report page configuration", def: "Page-level settings: size, type (16:9, letter, tooltip, custom), background, wallpaper, alignment.", speed: "Format → Page. Size + Background.", tip: "Set custom canvas size for big-screen monitors and tooltips.", practice: "Set a 1920×1080 custom canvas for executive dashboards on large displays." },
          ...
          { term: "Filter pane", def: "Side pane where authors and consumers configure visual-, page-, and report-level filters.", speed: "Visual / Page / Report level. Lock and hide individual filters.", tip: "Hide system filters from consumers; lock business filters to prevent edits.", practice: "Lock the report-level 'Year = current' filter so consumers can't accidentally remove it." },
          ...
          { term: "Visual calculations", def: "DAX expressions scoped to a single visual — running totals, ranks.", speed: "DAX scoped to a single visual. Use visual-scoped window functions (e.g., running totals, ranks).", tip: "Visual calculations use different context from standard DAX.", practice: "Use for running totals without creating model measures." },
      ...
      },
      {
        ...
        emoji: "🔹",
        terms: [
          { term: "Narrative visuals", def: "AI-generated natural language summaries of data trends.", speed: "Copilot generates text summary. Requires Fabric/Premium.", tip: "Requires Fabric or Premium capacity.", practice: "Review and customise Copilot-generated narratives." },
          { term: "Report page generation (Copilot)", def: "Copilot creates entire report pages from the model and a prompt.", speed: "Quality improves with good field names and descriptions.", tip: "Good field naming improves Copilot output quality.", practice: "Add descriptions to measures and tables to improve suggestions." },
          { term: "Copilot content suggestions", def: "Copilot proposes visuals, fields, and layouts for a new report page.", speed: "Home → Copilot → Suggest content.", tip: "Use 'Suggest content for a new page' to bootstrap a draft, then refine.", practice: "Generate a starter Sales overview page with Copilot, then tighten manually." },
          { term: "Semantic model summaries", def: "Copilot generates plain-English descriptions of the data model structure.", speed: "Plain English model overview. Useful for onboarding.", tip: "Useful for onboarding new report authors.", practice: "Use as a starting point, then refine with business terminology." },
        ]
      },
      {
        ...
        terms: [
          ...
          { term: "Page navigation", def: "Buttons, page navigators, and bookmarks used to move between report pages.", speed: "Insert → Buttons → Page navigator (auto-updates).", tip: "Page navigator visuals update automatically when pages are renamed/added.", practice: "Use a page navigator instead of hand-built button menus." },
          { term: "Sorting", def: "Visual-level setting that controls the order of categorical or numeric values.", speed: "More options → Sort axis. Or 'Sort By Column' on the field.", tip: "Use Sort By Column for months, weekdays, and custom buckets.", practice: "Set MonthName sort by MonthNumber so visuals show Jan→Dec order." },
          ...
          { term: "Selection pane", def: "Pane that lists every visual on the page and lets you reorder, group, hide, and set tab order.", speed: "View → Selection pane. Drag to reorder Z-index.", tip: "Group related visuals so bookmarks can show/hide them together.", practice: "Use the Selection pane to set keyboard tab order for accessibility." },
          ...
          { term: "Export settings", def: "Workspace/tenant settings controlling whether users can export to Excel, CSV, PDF, or underlying data.", speed: "File → Options → Report settings. Tenant admin can lock down.", tip: "Restrict 'Export underlying data' for sensitive reports; exports still respect report filters and RLS.", practice: "Disable underlying-data export on RLS-protected reports." },
          { term: "Personalized visuals", def: "Lets consumers swap fields, change visual type, and save personal views without altering the source.", speed: "Enable via File → Options (Current File) → Report settings, or in the Service report settings.", tip: "Personalisations save per user as bookmarks — published report stays clean.", practice: "Enable on exploratory reports so analysts can self-serve." },
        ]
      },
      {
        ...
        emoji: "🔹",
        terms: [
          { term: "Analyze feature", def: "Right-click insight that explains increases, decreases, and distribution differences using ML.", speed: "Right-click data point → Analyze → Explain the increase / decrease.", tip: "Surfaces statistically significant drivers without writing DAX.", practice: "Use 'Explain the decrease' on a sales drop to find the root-cause segment." },
          { term: "Grouping", def: "Combine selected category values into a single bucket directly from the field.", speed: "Right-click field → New group → List grouping.", tip: "Use to merge tail values into 'Other' for cleaner visuals.", practice: "Group small countries into 'Rest of World' for executive summaries." },
          { term: "Binning", def: "Buckets a continuous numeric or date field into equal-size bins.", speed: "Right-click numeric/date → New group → Bin size.", tip: "Useful for histogram-style analyses on age, revenue, latency.", practice: "Bin order amounts into £100 buckets to build a histogram." },
          { term: "Clustering", def: "Algorithmic segmentation of data points based on similarity (k-means).", speed: "Scatter or table → ... → Automatically find clusters.", tip: "Avoid on tiny datasets (<50 rows) — cluster quality is unstable.", practice: "Run clustering on RFM scores to discover customer segments automatically." },
          ...
          { term: "Error bars", def: "Visual indicators of variance / confidence around a measure on a chart.", speed: "Analytics pane → Error bars. Constant or by measure.", tip: "Use error bars to show forecast confidence or A/B test variance.", practice: "Add ±1 stdev error bars to weekly KPI line charts." },
        ]
      },
      {
    ...
    sections: [
      {
        ...
        terms: [
          ...
          { term: "Deployment pipelines", def: "Service feature for promoting content across Dev → Test → Production stages.", speed: "Capacity feature (Fabric/Premium). 3 stages. Selective deployment.", tip: "Use parameter rules to swap data sources between stages.", practice: "Set up a pipeline so Dev changes promote to Test then Prod with one click." },
        ]
      },
      {
        ...
        terms: [
          ...
          { term: "Data lineage", def: "Visual map showing data flow from sources to reports.", speed: "Workspace → Lineage view. Impact analysis.", tip: "Shows what would break if a semantic model changed.", practice: "Review lineage before changing shared datasets." },
        ]
      },
      {
      ...
      },
      {
        ...
        terms: [
          ...
          { term: "Semantic model permissions", def: "Controls who can read, build on, or reshare a semantic model.", speed: "Read. Build. Reshare. Separate from workspace roles.", tip: "Build permission allows creating new reports.", practice: "Grant Build permission to analysts who need to create reports." },
          { term: "Sensitivity labels", def: "Microsoft Purview labels applied to classify and protect data.", speed: "Purview labels. Propagate from semantic model to reports + exports.", tip: "Labels propagate from dataset to reports and exports.", practice: "Apply labels to datasets before publishing." },
        ]
      },
    ]
  },
